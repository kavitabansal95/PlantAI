...
def answer_plant_question(question: str, plants: list) -> str:
    ...
    if plants:
        # build plant context without nested/escaped quotes (avoids f-string parsing issues)
        plant_list = ", ".join(f"{p['name']} (watered every {p['watering_frequency_days']} days)" for p in plants)
        plant_context = f"The user has these plants: {plant_list}."
    else:
